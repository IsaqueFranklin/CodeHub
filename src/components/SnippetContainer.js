import React, { useContext } from 'react'
import {Link, withRouter} from 'react-router-dom'
import FirebaseContext from '../firebase/context'
import { Card, Container, Row, Col } from 'react-bootstrap'

function SnippetContainer({ post, showCount, history}) {

    const { firebase, user } = useContext(FirebaseContext)

    function handleVote() {
        if(!user) {
            history.push('/login')
        } else {
            const voteRef = firebase.db.collection('snippets').doc(post.id)
            voteRef.get().then(doc => {
                if(doc.exists) {
                    const previousStars = doc.data().stars;
                    const star = { staredBy: { id: user.uid, name: user.displayName } };
                    const updatedStars = [...previousStars, star]
                    const starCount = updatedStars.length;
                    voteRef.update({ stars: updatedStars, starCount})
                }
            })
        }
    }

    function handleDeleteSnippet() {
        const linkRef = firebase.db.collection('snippets').doc(post.id)
        linkRef.delete().then(() => {
            console.log(`O documento com ID ${post.id} foi deletado.`)
        }).then(() => {window.location.reload()})
    }

    const postedByAuthUser = user && user.uid === post.postedBy.id

    return (
        <Container className="">
            <Card className="cardposts left alig">
            <Row>
            <Col md="auto">
                <img src={post.thumbImg} className="ig" />
            </Col>
            <Col md="auto">
                <Card.Body className="">
                <div className="">
                    <div className="left">
                            <div className="">
                                <h5 className="title">{post.title}</h5>{" "}
                                <small>{post.description}</small>{" | "}
                                <small>by {post.postedBy.name}</small>{" | "}
        
                                <small onClick={handleVote}>{post.voteCount} stars  <img src="data:image/png;base64," className="star" /></small>{" "}
                           
                                {" "}<Link to={`/post/${post.id}`} className="">
                                    <button style={{ padding: 8 }}>Read now</button>
                                </Link>
                                {postedByAuthUser && (
                                    <>
                                    {" "}
                                    <span className="delete" onClick={handleDeleteSnippet}>
                                        Delete
                                    </span>
                                    {" | "}
                                    <Link to={`/edit/${post.id}`}>
                                        Update
                                    </Link>
                                    </>
                                )}
                            </div>
                    </div>
                </div>
                </Card.Body>
            </Col>
        </Row>
        </Card>
    </Container>
    )
}

export default withRouter(SnippetContainer);